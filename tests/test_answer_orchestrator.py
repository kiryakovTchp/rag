...
class TestAnswerOrchestrator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch(
            "services.answer.orchestrator.PGVectorIndex"
        ) as mock_index_class, patch(
            "services.answer.orchestrator.get_llm_provider"
        ) as mock_get_llm, patch(
            "services.answer.orchestrator.EmbeddingProvider"
        ) as mock_embedder_class, patch(
            "services.answer.orchestrator.WorkersAIReranker"
        ) as mock_reranker_class, patch(
            "services.answer.orchestrator.AnswerCache"
        ) as mock_cache_class:
    ...
    def test_generate_answer_success(self):
        ...
        self.mock_db.query.return_value.filter.return_value.in_.return_value.all.return_value = (
            mock_chunks
        )
    ...
    def test_generate_answer_with_tenant_logging(self):
        ...
        with patch("services.answer.orchestrator.log_answer_usage") as mock_log:
            result = self.orchestrator.generate_answer(
                "Test query", tenant_id="test_tenant"
            )
    ...
    def test_rerank_order_by_chunk_id(self):
        ...
        self.orchestrator.reranker.rerank.return_value = [
            1,
            0,
            2,
        ]  # Reorder: 2nd, 1st, 3rd
