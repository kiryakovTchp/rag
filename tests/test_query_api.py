...
class TestQueryAPI(unittest.TestCase):
    ...
    def test_query_api_end_to_end(self):
        ...
        if document_id:
            for attempt in range(60):
                ...
                embed_jobs = [
                    j
                    for j in document_status.get("jobs", [])
                    if j.get("type") == "embed"
                ]
                if embed_jobs and embed_jobs[0]["status"] == "done":
                    ...
                    self.fail(
                        f"Embed job failed: {embed_jobs[0].get('error', 'Unknown error')}"
                    )
                time.sleep(1)
            else:
    ...
    def test_query_validation(self):
        ...
        # Test invalid top_k
        response = self.client.post(
            "/query", json={"query": "test", "top_k": 300}  # > 200
        )
        self.assertEqual(response.status_code, 422)

        # Test invalid max_ctx
        response = self.client.post(
            "/query", json={"query": "test", "max_ctx": 5000}  # > 4000
        )
        self.assertEqual(response.status_code, 422)

    def test_rerank_disabled(self):
        """Test rerank when disabled."""
        response = self.client.post("/query", json={"query": "test", "rerank": True})
