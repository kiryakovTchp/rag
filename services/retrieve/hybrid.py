...
class HybridRetriever:
    ...
    def __init__(self, embed_provider: Optional[str] = None):
        """Initialize hybrid retriever."""
        self.embed_provider = embed_provider or os.getenv(
    ...
    def retrieve(
        self, query: str, top_k: int = 100, rerank_k: int = 20, use_rerank: bool = False
    ) -> list[ChunkWithScore]:
        ...
        results = []
        for (_, score), chunk in zip(search_results, chunks):
            if chunk:
    ...
    def _apply_reranking(
        self, query: str, results: list[ChunkWithScore], rerank_k: int
    ) -> list[ChunkWithScore]:
