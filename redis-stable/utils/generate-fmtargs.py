...
print(
    "#define ARG_N(" + ", ".join(["_" + str(i) for i in range(1, MAX_ARGS + 1, 1)]) + ", N, ...) N"
)
...
for i in range(4, MAX_ARGS + 1, 2):
    print(
        "#define COMPACT_FMT_{}(fmt, value, ...) fmt COMPACT_FMT_{}(__VA_ARGS__)".format(i, i - 2)
    )

print("\n#define COMPACT_VALUES_2(fmt, value) value")
for i in range(4, MAX_ARGS + 1, 2):
    print(
        "#define COMPACT_VALUES_{}(fmt, value, ...) value, COMPACT_VALUES_{}(__VA_ARGS__)".format(
            i, i - 2
        )
    )
