...
class VRANDMEMBERTest(TestCase):
    ...
    def test(self):
        ...
        ), "VRANDMEMBER with count on non-existent key should return empty array"
        ...
        assert len(decoded_results) == len(
            set(decoded_results)
        ), "Results should be unique with positive count"
        ...
        assert (
            len(result) == count
        ), "Should return only the available members when asking for more than exist"
        ...
        assert (
            isinstance(result, list) and len(result) == 0
        ), "VRANDMEMBER with count=0 should return empty array"
