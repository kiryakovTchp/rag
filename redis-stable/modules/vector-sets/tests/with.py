...
class VSIMWithAttribs(TestCase):
    ...
    def test(self):
        ...
        assert all(
            isinstance(item, bytes) for item in results_resp3
        ), "RESP3: Results should be byte strings"
        ...
        for i in range(0, len(results_resp2), 2):
            ...
            assert attr is None or isinstance(
                attr, bytes
            ), f"RESP2: Attribute at {i+1} should be None or bytes"
            if attr is not None:
        ...
        for item, value in results_resp3.items():
            ...
            assert isinstance(
                value, list
            ), f"RESP3: With both WITH options, value should be a list, got {type(value)}"
            assert (
                len(value) == 2
