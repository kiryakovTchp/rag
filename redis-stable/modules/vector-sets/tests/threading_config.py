...
class ThreadingConfigTest(TestCase):
    ...
    def get_config_value(self):
        ...
        try:
            result = self.redis.execute_command(
                "CONFIG", "GET", "vset-force-single-threaded-execution"
            )
            if len(result) >= 2:
    ...
    def set_config_value(self, value):
        ...
        try:
            ...
            result = self.redis.execute_command(
                "CONFIG", "SET", "vset-force-single-threaded-execution", str_value
            )
    ...
    def test_config_access_and_mutability(self):
        ...
        assert self.set_config_value(
            initial_value == "yes"
        ), "Should be able to restore original value"
    ...
    def test_vadd_without_cas(self, force_single_threaded=False):
        ...
        result = self.redis.execute_command(
            "VADD", self.test_key, "FP32", vec_bytes, f"{self.test_key}:item:1"
        )
    ...
    def test_vadd_with_cas(self, force_single_threaded=False):
        ...
        result = self.redis.execute_command(
            "VADD", self.test_key, "FP32", vec_bytes, f"{self.test_key}:item:cas", "CAS"
        )
    ...
    def test_vsim_with_nothread(self, force_single_threaded=False):
        ...
        assert isinstance(
            result, list
        ), f"VSIM with NOTHREAD should return a list, got {type(result)}"
        assert (
            len(result) <= 3
        ), f"VSIM with NOTHREAD should return at most 3 results, got {len(result)}"
    ...
    def test_threading_mode_comparison(self):
        ...
        assert (
            multi_threaded_card == single_threaded_card
    ...
    def test_nothread_override_behavior(self):
        ...
        for force_single in [False, True]:
            ...
            assert isinstance(
                result, list
            ), f"NOTHREAD should work with force_single={force_single}"
            assert (
                len(result) <= 2
            ), f"NOTHREAD should return ≤2 results with force_single={force_single}"
    ...
    def test(self):
        ...
        print(
            f"Initial vset-force-single-threaded-execution: {'yes' if initial_force_single else 'no'}"
        )
