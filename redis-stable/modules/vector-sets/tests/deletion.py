...
class VREM(TestCase):
        ...
        for level, neighbors in enumerate(links_result):
            ...
            for i in range(0, len(neighbors), 2):
                ...
                if score is not None:
                    neighbors_with_scores.append(
                        f"{len(neighbors_with_scores)+1}. {neighbor} ({score:.6f}){status}"
                    )
                else:
                    neighbors_with_scores.append(
                        f"{len(neighbors_with_scores)+1}. {neighbor}{status}"
                    )
    ...
    def test(self):
        ...
        new_results = self.redis.execute_command(
            "VSIM",
            self.test_key,
            "VALUES",
            dim,
        ...
        if failure:
            ...
            if failed_item in neighbors_before:
                print(
                    self.format_neighbors_with_scores(
                        neighbors_before[failed_item], items_to_remove=items_to_remove
                    )
                )
            else:
            ...
            current_links = self.redis.execute_command(
                "VLINKS", self.test_key, failed_item, "WITHSCORES"
            )
            if current_links:
                print(
                    self.format_neighbors_with_scores(
                        current_links, old_links=neighbors_before.get(failed_item)
                    )
                )
            else:
