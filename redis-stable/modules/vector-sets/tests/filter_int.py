...
class VSIMFilterAdvanced(TestCase):
    ...
    def test_recall_with_filter(self, filter_expr, ef=500, filter_ef=None):
        ...
        for i in range(0, len(redis_results), 2):
            key = (
                redis_results[i].decode()
                if isinstance(redis_results[i], bytes)
                else redis_results[i]
            )
        ...
        assert (
            abs(selectivity - true_selectivity) < 0.1
    ...
    def test(self):
        ...
        results["complex1"] = self.test_recall_with_filter(
        ...
        assert (
            high_ef_recall >= low_ef_recall * 0.8
        ...
        for expr, type_name in type_filters:
            try:
                _, _, query_time, result_count = self.test_recall_with_filter(expr)
                print(
                    f"{type_name:16} | {expr:30} | {result_count:5} results | {query_time*1000:.1f}ms"
                )
            except Exception as e:
    ...
    def create_dataset(self):
        ...
        for i in range(self.count):
            ...
            category = (
                ...
                else random.choice(["standard", "economy", "basic"])
            )
