...
class VSIMFilterExpressions(TestCase):
    ...
    def test(self):
        ...
        self.redis.execute_command(
            "VADD", self.test_key, "VALUES", 4, *[str(x) for x in vec1], f"{self.test_key}:item:1"
        )
        self.redis.execute_command(
            "VSETATTR",
            self.test_key,
        ...
        self.redis.execute_command(
            "VADD", self.test_key, "VALUES", 4, *[str(x) for x in vec2], f"{self.test_key}:item:2"
        )
        self.redis.execute_command(
            "VSETATTR",
            self.test_key,
        ...
        self.redis.execute_command(
            "VADD", self.test_key, "VALUES", 4, *[str(x) for x in vec3], f"{self.test_key}:item:3"
        )
        self.redis.execute_command(
            "VSETATTR",
            self.test_key,
            f"{self.test_key}:item:3",
            '{"age": 35, "name": "Charlie", "scores": [60, 65, 70], "city": "Seattle"}',
        )

        self.redis.execute_command(
            "VADD", self.test_key, "VALUES", 4, *[str(x) for x in vec4], f"{self.test_key}:item:4"
        )
        # Item 4 has no attribute at all

        self.redis.execute_command(
            "VADD", self.test_key, "VALUES", 4, *[str(x) for x in vec5], f"{self.test_key}:item:5"
        )
        self.redis.execute_command(
            "VSETATTR", self.test_key, f"{self.test_key}:item:5", "invalid json"
        )  # Intentionally malformed JSON

        # Basic equality with numbers
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".age == 25"
        )
        ...
        # Less than or equal
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".age <= 30"
        )
        ...
        # Boolean value
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".active"
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '"lic" in .name'
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '"ork" in .city'
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '"xyz" in .name'
        )
        assert len(result) == 0, "Expected 0 results for 'xyz' in name"

        # Off-by-one tests - substring at the beginning
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '"Ali" in .name'
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '"ice" in .name'
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '"A" in .name'
        )
        assert len(result) == 1, "Expected 1 result for single char 'A' in 'Alice'"

        # Off-by-one tests - empty string (should match all strings)
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '"" in .name'
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", '.name in ""'
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".age + 10 > 40"
        )
        assert len(result) == 1, "Expected 1 result for age + 10 > 40"

        # Arithmetic operations - multiplication
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".age * 2 > 60"
        )
        assert len(result) == 1, "Expected 1 result for age * 2 > 60"

        # Arithmetic operations - division
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".age / 5 == 5"
        )
        assert len(result) == 1, "Expected 1 result for age / 5 == 5"

        # Arithmetic operations - modulo
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".age % 2 == 0"
        )
        assert len(result) == 1, "Expected 1 result for age % 2 == 0"

        # Power operator
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".age ** 2 > 900"
        )
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".any_field"
        )
        assert f"{self.test_key}:item:4" not in [
            item.decode() for item in result
        ], "Item with no attribute should be excluded"

        # Malformed JSON
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".any_field"
        )
        assert f"{self.test_key}:item:5" not in [
            item.decode() for item in result
        ], "Item with malformed JSON should be excluded"
        ...
        result = self.redis.execute_command(
            ...
            *[str(x) for x in vec1],
            "FILTER",
            '(.age > 20 and .age < 40) and (.city == "Boston" or .city == "New York")',
        )
        ...
        assert set([item.decode() for item in result]) == set(
            expected_items
        ), "Expected item:1 and item:2 for the complex expression"
        ...
        result = self.redis.execute_command(
            "VSIM", self.test_key, "VALUES", 4, *[str(x) for x in vec1], "FILTER", ".scores"
        )
