...
def fill_redis_with_vectors(
    r: redis.Redis, key: str, count: int, dim: int, with_reduce: Optional[int] = None
) -> VectorData:
...
class TestCase:
    ...
    def setup_replication(self) -> bool:
        ...
        for attempt in range(max_attempts):
            ...
            if (
                repl_info.get("role") == "slave"
                and repl_info.get("master_host") == "127.0.0.1"
                and repl_info.get("master_port") == self.primary_port
                and repl_info.get("master_link_status") == "up"
            ):
...
def check_redis_empty(r, instance_name):
    ...
    try:
        ...
        if dbsize > 0:
            print(
                colored(
                    f"ERROR: {instance_name} Redis instance DB 9 is not empty (dbsize: {dbsize}).",
                    "red",
                )
            )
            print(
                colored(
                    "Make sure you're not using a production instance and that all data is safe to delete.",
                    "red",
                )
            )
            sys.exit(1)
...
def check_replica_running(replica_port):
    ...
    try:
        ...
        print(
            colored(
                "Replication tests will be skipped. Make sure to start the replica instance.",
                "yellow",
            )
        )
        return False
