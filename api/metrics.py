...
# Cost metrics
COST_USD = Counter("llm_cost_usd_total", "Total LLM cost in USD", ["provider", "model"])

# Queue metrics
CELERY_QUEUE_SIZE = Gauge(
    "celery_queue_size", "Number of tasks in Celery queue", ["queue_name"]
)

CELERY_WORKER_ACTIVE = Gauge(
    "celery_worker_active", "Number of active Celery workers", ["queue_name"]
)
...
QUOTA_EXCEEDED = Counter(
    "quota_exceeded_total", "Total quota violations", ["tenant_id", "quota_type"]
)
        ...
        async def send_wrapper(message):
            if message["type"] == "http.response.start":
                ...
                if status_code >= 400:
                    ...
                    ERROR_COUNT.labels(
                        method=method, endpoint=path, error_type=error_type
                    ).inc()
...
def record_token_usage(provider: str, model: str, direction: str, tokens: int):
    """Record token usage."""
    TOKEN_USAGE.labels(provider=provider, model=model, direction=direction).inc(tokens)
...
def record_rate_limit_exceeded(user_id: str, endpoint: str):
    ...
    RATE_LIMIT_EXCEEDED.labels(user_id=user_id, endpoint=endpoint).inc()


def record_quota_exceeded(tenant_id: str, quota_type: str):
    """Record quota violation."""
    QUOTA_EXCEEDED.labels(tenant_id=tenant_id, quota_type=quota_type).inc()
