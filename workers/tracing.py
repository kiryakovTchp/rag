...
class TracedTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        with create_span(f"celery.task.{self.name}", {
...
def record_ingest_job(tenant: str, job_type: str, status: str, duration: float):
    ...
    INGEST_JOB_DURATION.labels(tenant=tenant, job_type=job_type, status=status).observe(duration)

def record_embedding_job(tenant: str, model: str, text_count: int, duration: float):
    """Record embedding job metric."""
    EMBEDDING_DURATION.labels(tenant=tenant, model=model, text_count=text_count).observe(duration)

def record_rerank_job(tenant: str, model: str, candidate_count: int, duration: float):
    """Record rerank job metric."""
    RERANK_DURATION.labels(tenant=tenant, model=model, candidate_count=candidate_count).observe(duration)
